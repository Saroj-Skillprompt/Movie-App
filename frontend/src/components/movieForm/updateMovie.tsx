// Updated CreateMovie.tsx → UpdateMovie.tsx (with fix for FormData mutation)
...
import { useUpdateMovieMutation } from "@/api/movies/movie.mutations";
import { toast } from "sonner";
import { useQueryClient } from "@tanstack/react-query";

const genreOptions = ["Action", "Romantic", "Sci-Fi", "Drama", "Horror", "Comedy"];
...
const UpdateMovie = () => {
  ...
  const onSubmit = async (data: z.infer<typeof movieSchema>) => {
    ...
    mutation.mutate(
      ...
      {
        onSuccess: (res) => {
          toast.success(res.message || "Movie updated successfully!");
        ...
        onError: (err: unknown) => {
          if (
            ...
            toast.error(errorData.message || "Failed to update movie");
  ...
  return (
    <div className="min-h-screen pt-20 p-6 max-w-2xl mx-auto bg-white text-black dark:bg-gray-900 dark:text-white">
      ...
      <FormProvider {...methods}>
        ...
        >
          <InputField label="Title:" type="text" name="title" placeholder="Enter Title" />
          <InputField label="Description:" type="text" name="description" placeholder="Enter description" />
          <CheckboxGroupField label="Genres:" name="genre" options={genreOptions} />
          <SelectField label="Category:" name="category" options={categoryOptions} />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <InputField label="Director:" name="director" type="text" placeholder="Enter Director" />
            <InputField label="Release Year:" name="release_year" type="number" placeholder={"Select Release Year"} />
            <InputField label="Duration (minutes):" type="number" name="duration" placeholder={"Select Duration"} />
          </div>
          <DynamicInputListField label="Cast Members:" name="cast" className="space-y-3" />
          <FileUploadField name="poster_url" label="Movie Poster:" accept="image/*" />
          <FileUploadField name="video_url" label="Movie Video:" accept="video/*" />
          <Button
            type="submit"
